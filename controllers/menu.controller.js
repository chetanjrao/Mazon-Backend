...
module.exports = {
  ...
  getFeaturedFood: async (req, res, next) => {
    const restaurant_id = req.params["restaurantID"];
    const featured_food = await get_featured_restaurant_menu(restaurant_id);
    res.json(featured_food);
  },
  getParticularFood: async (req, res, next) => {
    const food_id = req.params["foodID"];
    const restaurant_id = req.params["restaurantID"];
    const food_details = await get_specific_food_details(
      restaurant_id,
      food_id
    );
  ...
  enableItem: async (req, res, next) => {
    ...
    const restaurant_id = req.params["restaurantID"];
    const updated_document = await enable_item(
      fId,
      restaurant_id,
      cId,
      res.locals["user_id"]
  ...
  disableItem: async (req, res, next) => {
    ...
    const cId = req.params["cId"];
    const restaurant_id = req.params["restaurantID"];
    const updated_document = await disable_item(
      fId,
      restaurant_id,
      cId,
      res.locals["user_id"]
  ...
  enableFeaturing: async (req, res, next) => {
    ...
    if (featured_food.length < 8) {
      const updated_document = await enable_featuring(
        fId,
        restaurant_id,
        cId,
        res.locals["user_id"]
  ...
  disableFeaturing: async (req, res, next) => {
    ...
    const cId = req.params["cId"];
    const restaurant_id = req.params["restaurantID"];
    const updated_document = await disable_featuring(
      fId,
      restaurant_id,
      cId,
      res.locals["user_id"]
