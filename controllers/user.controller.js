...
const MSG91 = new msg91("213839AHVCFomzdt5e274930P1");
...
const { create_transaction } = require("../services/transaction.service");
const {
...
const { storage } = require("../helpers/utils.helper");

const { generate_alphanumeric_otp } = require("../services/utils.service");
const {
  create_otp,
  validate_otp,
  invalidate_otp,
} = require("../services/otp.service");
const { mailer } = require("../helpers/utils.helper");
const {
  create_accesstoken,
...
const { get_offer_details, check_offer } = require("../services/offer.service");
const { get_complete_profile } = require("../services/user.service");
const { get_all_transactions } = require("../services/transaction.service");
...
const { create_wallet } = require("../services/wallet.service");
const {
  create_passkey,
  validate_passkey,
  invalidate_passkey,
...
module.exports = {
  signup: async (req, res, next) => {
    ...
    const current_user_check = await get_user_details_by_email_or_mobile(
      email,
      mobile
    );
            ...
            if (access_token_document != null) {
              ...
              if (now < expiry) {
                if (access_token_document["scopes"].indexOf(scope) === -1) {
                  ...
                  const current_user = await get_user_strict(
                    access_token_document["user"]
                  );
  ...
  verifyotp: async (req, res, next) => {
    ...
    if (is_valid_otp) {
      ...
      const passkey = await create_passkey(
        grant,
        "email",
        grant,
        ip,
  ...
  check_offer: async (req, res, next) => {
    ...
    if (offer_details != null) {
      ...
      if (user_avails < offer_details["max_user_avails"]) {
        const check_offer_validity = await check_offer(
          restaurant_id,
          offer_code
        );
    ...
    if (check.length < 3) {
      uploader(req, res, async function (err) {
        ...
        if (restaurant_id != undefined && dish_id != undefined) {
          const rating__ = await create_trending(
            dish_id,
            latitude,
            rating,
            review,
  ...
  profile: async (req, res, next) => {
    const user = await get_complete_profile(
      res.locals["user"],
      res.locals["user_id"]
    );
    res.json(user);
    ...
    if (user_document["referrer"] == "") {
      ...
      if (referrer_document != null) {
        ...
        await create_transaction(
          wallet["_id"],
          `Referred ${user_document["first_name"]}`,
          50,
          1,
          ip,
          `${
  ...
  tier: async (req, res, next) => {
    ...
    const food_ratings_count = food_ratings ? food_ratings.length : 0;
    const restaurant_ratings = await RestaurantRating.find({
      email: user,
    });
    const restaurant_ratings_count = restaurant_ratings
      ? restaurant_ratings.length
      : 0;
    ...
    if (
      food_ratings_count >= tier_details["food_ratings"] &&
      restaurant_ratings_count >= tier_details["restaurant_ratings"] &&
      inorder_length >= tier_details["inorders"] &&
      booking_count >= tier_details["bookings"] &&
      referral_length >= tier_details["referrals"]
    ) {
      is_claimable = true;
    }
    res.json({
  ...
  claim: async (req, res, next) => {
    ...
    if (is_claimable) {
      ...
      await create_transaction(
        wallet["_id"],
        "Completing Tier Tasks",
        tier_details["reward"],
        1,
        ip,
        "Mazon Technologies Pvt. Ltd.",
        user_id,
        user_agent
      );
